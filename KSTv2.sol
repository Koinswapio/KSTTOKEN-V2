...
contract Kswap is Ownable {
    ...
    string public constant name = "KOINSWAP";
    ...
    /// @notice Address of KST Treasury
    ...
    /**
     * @notice Construct a new KST token
     */
    constructor() {
    ...
    function approve(address spender, uint256 rawAmount) public returns (bool) {
        ...
        if (rawAmount == type(uint256).max) {
            ...
            amount = safe96(rawAmount, "KSTToken::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint256 rawAmount) external returns (bool) {
        uint96 amount = safe96(rawAmount, "KSTToken::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(
        ...
        if (spender != src && spenderAllowance != type(uint96).max) {
            uint96 newAllowance = sub96(
                spenderAllowance,
                amount,
                "KSTToken::transferFrom: transfer amount exceeds spender allowance"
            );
    ...
    function getPriorVotes(address account, uint256 blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "KSTToken::getPriorVotes: not yet determined");
    ...
    function burnFrom(address account, uint256 rawAmount) public {
        ...
        require(currentAllowance >= amount, "KSTToken: burn amount exceeds allowance");
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "KSTToken::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "KSTToken::delegateBySig: invalid nonce");
        require(block.timestamp <= expirye, "KSTToken::delegateBySig: signature expired");
    ...
    function _transferTokens(
        ...
        require(src != address(0), "KSTToken::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "KSTToken::_transferTokens: cannot transfer to the zero address");
        require(
            !_isBlackListed[src] && !_isBlackListed[dst],
            "KSTToken::_transferTokens: cannot transfer to/from blacklisted account"
        );
        require(tx.gasprice < gasPriceLimit, "KSTToken::_transferTokens: gasprice limit");
        ...
        if (sellLimitActive && _isSellLimited[src]) {
            ...
            uint96 totalBalance = add96(
                balances[src],
                withdrawnAmount,
                "KSTToken::_transferTokens: error with total balance"
            );
            ...
            require(
                ...
                "KSTToken::_transferTokens: withdraw more than 50% of balance"
            );
        ...
        if ((!isDex[dst] && !isDex[src]) || (_dexTaxExcempt[dst] || _dexTaxExcempt[src])) {
            if (!_isLimitExcempt[dst]) {
                require(
                    add96(
                        balances[dst], amount, 
                        "KSTToken::_transferTokens: exceds max per holder amount"
                    ) <= maxPerHolder,
                    "KSTToken::_transferTokens: final balance exceeds balance limit"
                );
            }
            balances[src] = sub96(balances[src], amount, "KSTToken::_transferTokens: transfer amount exceeds balance");
            balances[dst] = add96(balances[dst], amount, "KSTToken::_transferTokens: transfer amount overflows");
            emit Transfer(src, dst, amount);

            _moveDelegates(delegates[src], delegates[dst], amount);
        } else {
            require(!isTradingPaused, "KSTToken::_transferTokens: only liq transfer allowed");
            ...
            balances[taxTarget] = add96(
                balances[taxTarget], tax, "KSTToken::_transferTokens: transfer amount overflows"
            );
            balances[managementAddress] = add96(
                balances[managementAddress], teamTax, "KSTToken::_transferTokens: transfer amount overflows"
            );
            balances[dst] = add96(
                balances[dst],
                (amount - tax - teamTax),
                "KSTToken::_transferTokens: transfer amount overflows"
            );
    ...
    function _moveDelegates(
        ...
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "KSTToken::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "KSTToken::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, "KSTToken::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function updateTaxOnSell(uint8 _newTaxValue) public onlyOwner {
        require(_newTaxValue <= 80, "KSTToken::_adminFunctions: Tax cannot be greater than 80");
    ...
    function manageLimitExcempt(address[] memory users, bool[] memory _isUnlimited) public onlyOwner {
        require(users.length == _isUnlimited.length, "KSTToken::_adminFunctions: Array mismatch");
    ...
    function manageBlacklist(address[] memory users, bool[] memory _toBlackList) public onlyOwner {
        require(users.length == _toBlackList.length, "KSTToken::_adminFunctions: Array mismatch");
    ...
    function manageSellLimitExcempt(address[] memory users, bool[] memory _toLimit) public onlyOwner {
        require(users.length == _toLimit.length, "KSTToken::_adminFunctions: Array mismatch");
    ...
    function mintFor(address account, uint96 amount) public onlyOwner {
        require(minted + amount <= MAX_SUPPLY, "KSTToken::_adminFunctions: Mint more tokens than allowed");
